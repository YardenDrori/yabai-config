...
# Clean up empty dynamic spaces (per-display ws5-ws9) and unlabeled spaces
# Spaces with labels matching "d*-ws*" pattern or unlabeled spaces beyond 4 will be destroyed if empty
# Static spaces (d*-one through d*-four) are never destroyed

# Get current space to avoid destroying it
CURRENT_SPACE=$(yabai -m query --spaces --space | jq '.index')

# Get list of actual window space indices
WINDOW_SPACES=$(yabai -m query --windows | jq -r '[.[].space] | unique | .[]')
...
for display in $(yabai -m query --displays | jq -r '.[].index'); do
    ...
    yabai -m query --spaces | jq -r "[.[] | select(.display == $display) | .index] | sort | reverse | .[]" | while read -r SPACE_INDEX; do
        if [ "$SPACE_INDEX" != "$CURRENT_SPACE" ]; then
            ...
            # Skip if this is one of the first 4 spaces on this display (static)
